        ...
        with tf.variable_scope(self.network_name):
            with slim.arg_scope(self.arg_score()):
                ...
                with tf.variable_scope("merge_feature"):
                    #  a. 获取正向执行网络中执行序列的最后一个时刻的输出值作为正向的特征
                    # 实现方式一：先将output_fw反转，然后获取output_fw[:,0,:]即为最终结果
                    # 实现方式二：使用tf.gather_nd获取不固定索引位置的向量信息
                    batch_size = tf.shape(output_fw)[0]  # 批次大小
                    indices_fw = tf.concat([
                        tf.reshape(tf.range(batch_size), shape=(-1, 1)),  # 样本索引, [0,N-1]
                        tf.reshape(sequence_length - 1, shape=(-1, 1))  # 样本长度最后一个时刻的索引值, 每个样本的长度信息
                    ], axis=-1)
                    features_fw = tf.gather_nd(output_fw, indices_fw)

                    # b. 获取反向执行网络中执行序列的最后一个时刻的输出值作为反向的特征，也就是真实序列中的第0个时刻
                    features_bw = output_bw[:, 0, :]
